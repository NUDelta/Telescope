...
module.exports = Observable.extend({
  ...
  createBin: function (req, res, next) {
     var bin = utils.extract(req.body, 'html', 'css', 'javascript', 'fondue', 'settings');

    if (bin.html) {
      bin.html = beautify_html(bin.html, {
        "indent_size": 2,
    ...
    if (bin.fondue){
      bin.fondue.traces = JSON.parse(bin.fondue.traces);
      bin.fondue.scripts = JSON.parse(bin.fondue.scripts);
    }

    bin.javascript = "var foo = true;";
  ...
  templateFromBin: function (bin) {
    var template = utils.extract(bin, 'html', 'css', 'javascript', 'fondue');

    'html css javascript fondue'.split(' ').forEach(function (panel) {
    ...
    //this.writeJSToUnravelDemoFile(template);

    template.url = this.helpers.jsbinURL(bin); //.permalink;
    return template;
  },

  writeJSToUnravelDemoFile: function(template){
    ...
    fondue.scripts = _.chain(fondue.scripts).sortBy(function (script) {
      return script.order;
    }).where({builtIn:false}).value();
    ...
    var fs = require('fs');
    fs.writeFile("/code/unravel/demo/js.js", extractedHeadJS + extractedBodyJS, function (err) {
      if (err) {
